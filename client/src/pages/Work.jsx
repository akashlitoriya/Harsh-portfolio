...
const Work = () => {
  ...
  return (
    <div className='min-h-screen h-full text-5xl font-bold flex justify-center items-center snap-start'>
      {/* Container  */}
      <div className='w-11/12 md:w-4/5 md:min-h-[550px] flex flex-col justify-center items-center p-4 md:p-10 bg-white bg-opacity-5 backdrop-blur-md rounded-xl'>
        {/* Heading  */}
        <motion.div initial={{scale: 0}} whileInView={{scale:1}} transition={{delay:0.3, duration:0.3}} className='w-max'>
          <h1 className='text-white text-2xl lg:text-4xl w-fit'>Selected Work</h1>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 items-center gap-5 text-base mt-3 md:mt-6 border-0 border-green-500'>
             {
              categoryList.length > 0 && categoryList.map((category, index) =>(
                <motion.div onClick={() => navigate(category.navigate)} initial={{opacity:0}} animate={{opacity:100}} transition={{duration:0.3, delay: 0.3}} key={category.id} className={`flex flex-col justify-center items-center cursor-pointer h-32 md:w-60 md:h-40 border-0 rounded-lg border-blue-500 overflow-hidden`}>
