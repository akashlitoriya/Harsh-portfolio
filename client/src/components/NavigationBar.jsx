...
const NavigationBar = ({section}) => {
  ...
  return (
    <div className='h-4/6 relative w-full'>
        ...
        <div className='w-full flex flex-col justify-evenly h-full p-2 font-Montserrat text-xl'>
            {
                navItems.length > 0 && navItems.map((item)=>(
                    <div key={item.id} className={`w-full flex ${item.id === section? "flex-row" : "flex-row-reverse"} justify-around items-center gap-4`}>
